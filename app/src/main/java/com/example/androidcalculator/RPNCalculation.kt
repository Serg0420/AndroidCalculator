...
class RPNCalculation {
    ...
    fun calculate(input: String, contxt: Context): String {
        ...
        //возвращаем результат если ошибок нет, если есть кидаем тост
        return if (msgError.isEmpty()){
            result.pop()
        }else{
            Toast.makeText(contxt,msgError,Toast.LENGTH_SHORT).show()
            "Error"
        }
    }
    ...
    private fun toRPN(expr: String): Stack {
        ...
        // если сделать так expr.forEach { ch->, то break и continue не работают
        for (ch: Char in expr) {
